...
class FacialPreprocessor:
    ...
    def preprocess_and_save(
        self, source_path: str, output_path: str, source_is_array: bool = False
    ) -> str:
        try:
            if source_is_array:
                import cv2
            ...
            logger.error(
                f"Error preprocessing and saving {source_path} to {output_path}: {str(e)}"
            )
            raise ValueError(f"Failed to preprocess and save image: {str(e)}")

    def preprocess_batch(
        self, source_dir: str, output_dir: str, person_name: Optional[str] = None
    ) -> int:
    ...
    def preprocess_all_in_place(self, data_dir: str) -> dict:
        ...
        results = {"total_processed": 0, "total_failed": 0, "by_person": {}}
        ...
        for person_dir in data_path.iterdir():
            ...
            logger.info(
                f"Person '{person_name}': {processed_count} processed, {failed_count} failed"
            )

        logger.info(
            f"Total: {results['total_processed']} processed, {results['total_failed']} failed"
        )
        return results
