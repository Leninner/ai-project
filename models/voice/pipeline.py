...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train and evaluate voice recognition classifier"
    )
    parser.add_argument(
        "--classifier",
        "-c",
    ...
    print(
        f"   └─ Usuarios etiquetados como desconocidos: {np.sum(unknown_mask)}/{len(y_test)}"
    )
    ...
    if np.sum(known_mask) > 0:
        print(
            f"   ├─ Confianza promedio (conocidos): {np.mean(confidences[known_mask]):.2%}"
        )
    if np.sum(unknown_mask) > 0:
        print(
            f"   └─ Confianza promedio (desconocidos): {np.mean(confidences[unknown_mask]):.2%}"
        )
    ...
    plt.title(
        f"Matriz de confusión - SpeechBrain + {classifier_name} (Umbral: {CONFIDENCE_THRESHOLD})"
    )
    plt.colorbar()
