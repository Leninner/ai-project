...
class TrainingService:
    FACIAL_PIPELINE = (
        Path(__file__).parent.parent.parent.parent / "models" / "facial" / "pipeline.py"
    )
    VOICE_PIPELINE = (
        Path(__file__).parent.parent.parent.parent / "models" / "voice" / "pipeline.py"
    )
    ...
    def trigger_async_training(
        cls,
        facial_classifier_type: Optional[str] = None,
        voice_classifier_type: Optional[str] = None,
    ) -> None:
        if cls.is_training():
            logger.warning(
                "Training already in progress, skipping new training request"
            )
            return
        ...
        thread = threading.Thread(
            target=cls._train_models, args=(facial_type, voice_type), daemon=True
        )
        thread.start()

    @classmethod
    def _train_models(
        cls, facial_classifier_type: str, voice_classifier_type: str
    ) -> None:
        with cls._training_lock:
            if cls._is_training:
                return
        ...
        try:
            logger.info(
                f"Starting facial recognition model training with {facial_classifier_type.upper()} classifier..."
            )
            cls._train_facial_model(facial_classifier_type)

            logger.info(
                f"Starting voice recognition model training with {voice_classifier_type.upper()} classifier..."
            )
    ...
    def _train_voice_model(cls, classifier_type: str = None) -> None:
        ...
        if classifier_type not in ["nn"]:
            raise ValueError(
                f"Invalid classifier type: {classifier_type}. Must be 'nn'"
            )
