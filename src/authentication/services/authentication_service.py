...
class AuthenticationService:
    ...
    def authenticate(
        self, username: str, video_file: UploadedFile, request
        ...
        try:
            image_temp_path, voice_temp_path = (
                self.video_processor.process_authentication_video(video_file)
            )

            is_valid, error_message = self.biometric_validator.validate_all(
                voice_temp_path, image_temp_path, username
            )

            if not is_valid:
                logger.warning(
                    f"Biometric validation failed for {username}: {error_message}"
                )
