...
class VideoProcessor:
    FACIAL_DATA_DIR = (
        Path(__file__).parent.parent.parent.parent.parent / "models" / "facial" / "data"
    )
    VOICE_DATA_DIR = (
        Path(__file__).parent.parent.parent.parent.parent / "models" / "voice" / "data"
    )
    ...
    def process_registration_video(self, video_file, username: str) -> Tuple[int, int]:
    ...
    def process_authentication_video(self, video_file) -> Tuple[str, str]:
        temp_video_path = None
        ...
        try:
            temp_video_path = self.file_handler.save_uploaded_file(video_file)
            temp_frame_path = tempfile.NamedTemporaryFile(
            ...
            _, emb = self.media_processor.extract_voice_embedding_from_video(
                temp_video_path, output_audio_path=temp_audio_path
            )
    ...
    def extract_frames_and_audio(
        self, video_path: str, username: str
        ...
        frame_paths = self.frame_extractor.extract_frames(
            video_path, user_facial_dir, existing_frames
        )

        audio_paths = self.audio_extractor.extract_for_registration(
            video_path, user_voice_dir, existing_audio
        )
