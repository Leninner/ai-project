...
facial_models_path = (
    Path(__file__).parent.parent.parent.parent.parent / "models" / "facial"
)
...
class VideoFrameExtractor:
    ...
    def extract_single_face(
        self, video_path: str, min_duration_seconds: float = 3.0
    ) -> Optional[np.ndarray]:
        temp_mp4_path = None
        ...
        try:
            ...
            effective_min_duration = (
                min_duration_seconds - self.DURATION_TOLERANCE_SECONDS
            )
    ...
    def _validate_video_duration(
        self, video_duration: float, total_frames: int
    ) -> None:
    ...
    def _extract_faces_from_video(
        ...
        i = 0
        while (
            len(frame_paths) < self.REQUIRED_FRAMES and frames_attempted < max_attempts
        ):
