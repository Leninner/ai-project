...
class VideoAudioExtractor:
    ...
    def extract_for_registration(
        self, video_path: str, output_dir: Path, start_index: int
    ) -> List[str]:
        temp_audio = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
        temp_audio.close()

        try:
            AudioProcessor.extract_audio_with_noise_reduction(
                video_path, temp_audio.name
            )
    ...
    def extract_for_authentication(self, video_path: str, output_path: str) -> None:
        AudioProcessor.extract_audio_with_noise_reduction(video_path, output_path)
        self._validate_audio_duration(output_path)

    def _split_audio(
        self, audio_path: str, output_dir: Path, start_index: int
    ) -> List[str]:
    ...
    def _validate_audio_segments(
        self, audio_paths: List[str], audio_duration: float
    ) -> None:
    ...
    def _validate_audio_duration(self, audio_path: str) -> None:
        ...
        effective_min_duration = (
            self.MIN_AUDIO_DURATION_SECONDS - self.DURATION_TOLERANCE_SECONDS
        )
