...
class VoiceModelLoader(BaseModelLoader):
    ...
    def _get_nn_paths(self) -> Tuple[Path, Path]:
        return (
            BiometricConfig.VOICE_NN_MODEL_PATH,
            BiometricConfig.VOICE_NN_LABEL_ENCODER_PATH,
        )
    ...
    def _create_nn_model(self, num_classes: int) -> nn.Module:
        ...
        try:
            ...
            nn_classifier_path = (
                models_dir / "voice" / "classifiers" / "nn_classifier.py"
            )
            spec = importlib.util.spec_from_file_location(
                "nn_classifier", nn_classifier_path
            )
        ...
        return VoiceClassifier(
            embedding_dim=self._get_embedding_dim(), num_classes=num_classes
        )

    def _validate_paths(self) -> None:
        if self.classifier_type == "cnn":
            raise NotImplementedError(
                "CNN classifier not supported for voice recognition"
            )
        else:
            nn_model_path, label_encoder_path = self._get_nn_paths()
            if not nn_model_path.exists() or not label_encoder_path.exists():
                raise ValueError(
                    "Voice Neural Network models not found. Please train the models first."
                )
...
class FacialModelLoader(BaseModelLoader):
    ...
    def _get_nn_paths(self) -> Tuple[Path, Path]:
        return (
            BiometricConfig.FACIAL_NN_MODEL_PATH,
            BiometricConfig.FACIAL_NN_LABEL_ENCODER_PATH,
        )
    ...
    def _create_nn_model(self, num_classes: int) -> nn.Module:
        ...
        try:
            ...
            nn_classifier_path = (
                models_dir / "facial" / "classifiers" / "nn_classifier.py"
            )
            spec = importlib.util.spec_from_file_location(
                "nn_classifier", nn_classifier_path
            )
    ...
    def _validate_paths(self) -> None:
        if self.classifier_type == "cnn":
            ...
            if not nn_model_path.exists() or not label_encoder_path.exists():
                raise ValueError(
                    "Facial Neural Network models not found. Please train the models first."
                )
