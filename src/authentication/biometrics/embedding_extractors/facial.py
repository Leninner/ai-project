    ...
    def extract(self, image_path: str) -> List[float]:
        ...
        if img_array.shape == (160, 160, 3):
            ...
            if img_cropped is None:
                raise ValueError(
                    "No se detectó un rostro en la imagen. Por favor, asegúrate de que tu cara esté completamente visible y bien iluminada."
                )
        ...
        return (
            embedding_numpy.tolist()
            if hasattr(embedding_numpy, "tolist")
            else embedding_numpy
        )
