...
class BiometricValidator:
    ...
    def validate_voice(
        self, audio_path: str, expected_name: str
    ) -> Tuple[bool, str, float]:
        logger.debug("   → Delegating voice validation to VoiceValidator")
        return self.voice_validator.validate(audio_path, expected_name)

    def validate_facial(
        self, image_path: str, expected_name: str
    ) -> Tuple[bool, str, float]:
        logger.debug("   → Delegating facial validation to FacialValidator")
        return self.facial_validator.validate(image_path, expected_name)

    def validate_all(
        self, audio_path: str, image_path: str, expected_name: str
    ) -> Tuple[bool, str]:
        logger.info("")
        logger.info("═" * 60)
        logger.info("🔐 Complete Biometric Validation")
        ...
        logger.info("─" * 40)
        voice_valid, voice_msg, voice_conf = self.validate_voice(
            audio_path, expected_name
        )

        if not voice_valid:
            logger.warning("")
            logger.warning("═" * 60)
            logger.warning("✗ Complete biometric validation FAILED")
            logger.warning("   └─ Reason: Voice validation did not pass")
        ...
        logger.info("─" * 40)
        facial_valid, facial_msg, facial_conf = self.validate_facial(
            image_path, expected_name
        )
        ...
        if not facial_valid:
            ...
            logger.warning("═" * 60)
            logger.warning("✗ Complete biometric validation FAILED")
            logger.warning("   └─ Reason: Facial validation did not pass")
            logger.warning("═" * 60)
            return False, facial_msg

        logger.info("")
        logger.info("═" * 60)
        logger.info("✓ Complete biometric validation SUCCESSFUL")
        logger.info(f"   └─ User: {expected_name}")
